...
- Applies 4 quality filters to ensure clean data
- Converts tables to Excel format
- Provides detailed error reporting

**Quality Filters:**
Tables are automatically rejected if they:
1. **Empty** - Contains no data
2. **Too Small** - Less than 2 rows × 2 columns
3. **Too Sparse** - More than 40% empty cells
4. **Noise** - More than 30% punctuation-only cells (-, ., |, etc.)
...
2. Backend (`pdf_service.py`) processes PDF:
   ...
   - Applies OCR if needed (optional: requires Tesseract)
   - Filters tables using quality constraints
   - Tracks drop reasons for rejected tables
3. Extracted tables saved to `outputs/<job_id>/`
4. User sees detailed status: "Extracted 2 table(s). Dropped 5. Reasons: 3 too sparse, 2 punctuation noise."
5. User downloads Excel file

**New Features (v1.6):**
- ✅ Quality constraints info box on extraction page
- ✅ Detailed error reporting (shows why tables were rejected)
- ✅ Database schema updated with `extraction_details` column
- ✅ Removed `short_headers` constraint (allows abbreviated column names)
- ✅ Placeholder sheet when 0 tables extracted (prevents Excel errors)
- ✅ Fixed deprecated `applymap` warning

**Key Files:**
- `pdf_service.py` - Extraction logic with quality filters
- `pages/pdf-extract.html` - Upload interface with info box
- API: `POST /api/pdf/upload`, `GET /api/pdf/jobs/{job_id}/status`
...
CREATE TABLE uploads (
    ...
    created_at TEXT,
    error_msg TEXT,
    extraction_details TEXT  -- JSON: tables_extracted, tables_dropped, drop_reasons
);
```
...
- `extraction_details`: JSON string with extraction statistics
  ```json
  {
    "tables_extracted": 2,
    "tables_dropped": 5,
...
- **v1.6**: Enhanced PDF extraction with quality filters and detailed error reporting
  - Added quality constraints info box
  - Detailed extraction statistics (tables extracted/dropped/reasons)
